...
const App: React.FC = () => {
  ...
  const addToCart = (product: Product) => {
    setCart(prev => {
      ...
      if (existing) {
        toast.success(isAr ? `تم تحديث الكمية لـ ${product.name}` : `Updated quantity for ${product.name}`, {
          icon: '🛒',
        });
        return prev.map(i => i.id === product.id ? { ...i, quantity: i.quantity + 1 } : i);
      } else {
        toast.success(isAr ? `تم إضافة ${product.name} إلى السلة` : `Added ${product.name} to cart`, {
          icon: '✅',
        });
                ...
                >
                  <AnimatePresence>
                    {(selectedHomeCategory === 'All' && !searchQuery ? otherProducts : filteredProducts).length > 0 ? (
                      (selectedHomeCategory === 'All' && !searchQuery ? otherProducts : filteredProducts).map((product, index) => (
                        <motion.div
                          key={product.id}
                      ...
                      <motion.div
                        className="col-span-full py-24 flex flex-col items-center opacity-30 space-y-4"
